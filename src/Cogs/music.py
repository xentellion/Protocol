...
class Music(commands.Cog):
    ...
    def search_youtube(self, url):
        with YoutubeDL(self.YDL_OPTIONS) as ydl:
            try:
                info = ydl.extract_info("ytsearch:%s" % url, download=False)["entries"][
                    0
                ]
                return Song(info)
            except:
    ...
    async def play_music(self, ctx):
        if len(self.music_queue) > 0:
            ...
            time = self.music_queue[0][0].time
            await ctx.send(
                f"**Now playing:** {self.music_queue[0][0].title} `[{time//60}:{time%60}]`"
            )
            self.vc.play(
                discord.FFmpegPCMAudio(m_url, **self.FFMPEG_OPTIONS),
                after=lambda e: self.play_next(),
    ...
    async def p(self, ctx, *args):
        ...
        async with ctx.typing():
            ...
            if song == 0:
                ...
                time = self.music_queue[0][0].time
                await ctx.send(
                    f"🎵 **{self.music_queue[0][0].title}** `[{time//60}:{time%60}]` is added to the queue by **{ctx.message.author.name}**"
                )
